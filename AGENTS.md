...
- The app uses read-only public market data plus the configured NexaTrade
...
- `@nexatrade/ui` is a bun-workspace package consumed source-direct: the
  `"svelte"` export condition points at `src/index.ts` and there is no build
  step. Portal's `vite.config.ts` sets `ssr: { noExternal: ["@nexatrade/ui"] }`
...
- Entry-point invariant: server-side code (`apps/portal/src/lib/server/`,
  `routes/og/`) imports ONLY `@nexatrade/ui/tokens` or
  `@nexatrade/ui/format` (pure TS, no `.svelte`). The component barrel
  `@nexatrade/ui` and `tokens.css` are client/SSR-component territory.
...
- Two formatter dialects by design: `@nexatrade/ui/format` renders "—"
...
- Production domain: `nexatrade.trade`; the legacy `traderralph.com`,
