<script lang="ts">
  import { AssetTable, SiteFooter, SiteNav, TabNav } from "@nexatrade/ui";
...
<svelte:head>
  <title>{data.title} — trade on Solana | NexaTrade</title>
  <meta name="description" content={data.blurb} />
  <link rel="canonical" href={`https://nexatrade.trade/${data.hub}`} />
  <meta property="og:title" content={`${data.title} — trade on Solana | NexaTrade`} />
  <meta property="og:description" content={data.blurb} />
  <meta property="og:image" content={`https://nexatrade.trade/og/${data.hub}.png`} />
