<script lang="ts">
  import { NewsItem, SiteFooter, SiteNav, StatCard } from "@nexatrade/ui";
  import { fmtPct, fmtUsd } from "@nexatrade/ui/format";
...
<svelte:head>
  <title>{asset.symbol} — {asset.name} price, news & trading | NexaTrade</title>
  <meta
    name="description"
  ...
  />
  <link rel="canonical" href={`https://nexatrade.trade/${asset.slug}`} />
  <meta property="og:title" content={`${asset.symbol} — ${asset.name} | NexaTrade`} />
  <meta
    property="og:description"
    content={`Live ${asset.name} (${asset.symbol}) price, desk analysis and news. Trade ${asset.symbol} on Solana with USDC.`}
  />
  <meta property="og:image" content={`https://nexatrade.trade/og/${asset.slug}.png`} />
