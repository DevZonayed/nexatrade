<script lang="ts">
  ...
  const imageUrl = $derived(`https://nexatrade.trade/og/position.png?${data.query}`);
...
<svelte:head>
  <title
    ...
    {pnlText} | NexaTrade</title
  >
  <meta name="robots" content="noindex" />
  <meta
    property="og:title"
    content={`${share.paper ? "Paper trade: " : ""}${share.side.toUpperCase()} ${share.symbol} ${pnlText} on NexaTrade`}
  />
  <meta
