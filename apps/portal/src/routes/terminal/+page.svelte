<script lang="ts">
  ...
  import { OpenBetaBanner } from "@nexatrade/ui";
  import { colors } from "@nexatrade/ui/tokens";
  import {
    clearJournal,
    entriesToday,
    loadJournal,
  ...
  // self-serve — the app re-runs the referral onboarding with the NexaTrade
  ...
  // Legacy key name kept across the NexaTrade rebrand.
  ...
  $: docTitle = (() => {
    if (tradeMode === "spot" && spotAsset) {
      return spotAsset.price !== null
        ? `${formatPrice(spotAsset.price)} ${spotAsset.symbol} · NexaTrade`
        : `${spotAsset.symbol} · NexaTrade`;
    }
    return latestPrice !== null
      ? `${formatPrice(latestPrice)} ${selectedSymbol}-PERP · NexaTrade`
      : "NexaTrade";
  })();
  ...
  function sharePhoenixPosition(position: PhoenixPosition, mids: Record<string, number>): void {
    ...
    const text = paperMode
      ? `Paper trade: ${dir} ${position.symbol} on NexaTrade (simulated — not real funds)`
      : `${dir} ${position.symbol} on NexaTrade`;
    window.open(
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Frontend-only NexaTrade SvelteKit terminal."
  />
</svelte:head>
